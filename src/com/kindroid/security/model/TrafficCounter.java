...
public class TrafficCounter extends AbstractModel {
	...
	public TrafficCounter(long id, Interface inter) {
		this(inter);
		mId = id;
		mWhere = Counters._ID + "=" + id;
	}

	public TrafficCounter(Interface inter) {
		mInterface = inter;
	...
	private static String prettyBytes(long value) {
		...
		sb.append(' ');
		sb.append(KindroidSecurityApplication.BYTE_UNITS[i]);
		return sb.toString();
	}
	...
	private String prettyType() {
		...
		case SINGLE_DAY:
			...
			return KindroidSecurityApplication.COUNTER_SINGLE_DAY[number]
					.toString();
		case LAST_MONTH:
			n = getProperty(NUMBER, "0");
			number = Integer.valueOf(n);
			return KindroidSecurityApplication.COUNTER_LAST_MONTH[number]
					.toString();
		case LAST_DAYS:
			n = getProperty(NUMBER, "0");
			number = Integer.valueOf(n);
			return KindroidSecurityApplication.COUNTER_LAST_DAYS[number]
					.toString();
		case MONTHLY:
			n = getProperty(NUMBER, "0");
			number = Integer.valueOf(n);
			String t = KindroidSecurityApplication.COUNTER_TYPES[mType]
					.toString();
			String m = KindroidSecurityApplication.COUNTER_MONTHLY[number]
					.toString();
			if (number == 0) {
				return t;
			}
		...
		case WEEKLY:
			n = getProperty(NUMBER, "0");
			number = Integer.valueOf(n);
			t = KindroidSecurityApplication.COUNTER_TYPES[mType].toString();
			m = KindroidSecurityApplication.COUNTER_WEEKLY[number].toString();
			if (number == 0) {
				return t;
			}
			return t + " (" + m + ")";
		default:
			return KindroidSecurityApplication.COUNTER_TYPES[mType].toString();
		}
	}
	...
	private synchronized void loadBytes(SQLiteDatabase db) {
		...
		case SINGLE_DAY:
			...
			break;
		case TrafficCounter.LAST_MONTH:
			...
			mStringDate[1] = DatabaseHelper.getLocaleDate(c);
			break;
		case TrafficCounter.MONTHLY:
			value = getProperty(DAY, "1981-01-01");
			number = Integer.valueOf(getProperty(NUMBER, "0"));
			if (value.equals("1981-01-01")) {
				Calendar now = Calendar.getInstance();
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				value = sdf.format(now.getTime());
			}
			a = value.split("-");
			...
			if (v == day_mon) {
				...
				c.set(Calendar.DAY_OF_MONTH, day_mon);
				setBytes(mInterface.getInterfaceBytes(db, c, Calendar.getInstance()));
			}else if(v<day_mon){
				cc = Calendar.getInstance();
				if(c.get(Calendar.MONTH)==0){
					cc.set(Calendar.YEAR,c.get(Calendar.YEAR)-1);
					cc.set(Calendar.MONTH,11);
					cc.set(Calendar.DAY_OF_MONTH,day_mon);
				}else{
					cc.set(Calendar.MONTH,c.get(Calendar.MONTH)-1);
					cc.set(Calendar.DAY_OF_MONTH, 1);
					int day=cc.getActualMaximum(Calendar.DAY_OF_MONTH);
					if(day>=day_mon){
						cc.set(Calendar.DAY_OF_MONTH, day_mon);
					}else{
